...
export default function Home() {
  ...
  const productImages = settings.photo_urls.map((src, index) => ({ src, label: index === 0 ? "Couverture" : `Photo ${index + 1}` }));
    ...
    <main className="min-h-screen bg-[#f7f5f0] text-[#12233f]">
      ...
      <section id="inicio" className="mx-auto grid max-w-7xl gap-10 px-5 py-10 lg:grid-cols-[1.05fr_.95fr] lg:px-8 lg:py-16">
        <div className="relative aspect-[4/3] self-start overflow-hidden rounded-[2rem] bg-[#123f91] shadow-2xl shadow-blue-950/15">
          {productImages.length ? <Image src={productImages[Math.min(activeImage, productImages.length - 1)].src} alt={`${productImages[Math.min(activeImage, productImages.length - 1)].label} du livre Pale Panyol Fasil`} fill priority sizes="(min-width: 1024px) 52vw, 100vw" className="object-contain object-center" /> : <div className="grid h-full place-items-center px-8 text-center text-white"><div><BookOpen className="mx-auto h-20 w-20" /><p className="mt-4 text-xl font-black">Photos réelles bientôt disponibles</p></div></div>}
          ...
          {productImages.length > 0 && <div className="absolute bottom-4 left-4 rounded-full bg-white/90 px-4 py-2 text-sm font-bold">{Math.min(activeImage, productImages.length - 1) + 1} / {productImages.length} · {productImages[Math.min(activeImage, productImages.length - 1)].label}</div>}
        </div>
        <div id="livre" className="flex flex-col justify-center">
          ...
          <div className="mt-6 flex flex-wrap gap-2">{[`${settings.book_pages} pages`, "Espagnol expliqué en créole", "Exercices pratiques", "Couverture souple"].map((item) => <span key={item} className="rounded-full border border-[#123f91]/15 bg-white px-4 py-2 text-sm font-bold">{item}</span>)}</div>
      ...
      <Dialog open={checkoutOpen} onOpenChange={setCheckoutOpen}><DialogContent className="max-h-[92vh] max-w-2xl overflow-y-auto rounded-3xl p-0"><DialogHeader className="bg-[#102c64] p-6 text-left text-white"><DialogTitle className="font-serif text-3xl">Votre commande</DialogTitle></DialogHeader><div className="space-y-5 p-6">
