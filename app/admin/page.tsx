...
type Data = { orders: Row[]; customers: Row[]; locations: Row[]; movements: Row[]; batches: Row[]; payments: Row[]; shipments: Row[]; budgets: Row[]; campaigns: Row[]; integrations: Row[] };
const emptyData: Data = { orders: [], customers: [], locations: [], movements: [], batches: [], payments: [], shipments: [], budgets: [], campaigns: [], integrations: [] };
    ...
    <div className="lg:pl-[280px]">
      ...
      <div className="mx-auto max-w-[1500px] space-y-6 p-4 sm:p-7">
        ...
        {section === "integrations" && <>
          <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><b>Protection des secrets :</b> ce panneau enregistre uniquement les identifiants publics et l’état des intégrations. Les clés privées doivent rester dans les variables chiffrées Vercel.</div>
          <div className="grid gap-4 lg:grid-cols-2">{data.integrations.map((integration) => <IntegrationCard key={str(integration.provider)} item={integration} saving={saving} onSave={(payload) => void run("PATCH", { action: "integration", ...payload }, `${providerNames[str(integration.provider)]} mis à jour`)} />)}</div>
        </>}
      </div>
    </div>
  </main>;
...
function IntegrationCard({ item, saving, onSave }: { item: Row; saving: boolean; onSave: (payload: Row) => void }) { const [enabled, setEnabled] = useState(Boolean(item.enabled)); const [environment, setEnvironment] = useState(str(item.environment) || "sandbox"); const [identifier, setIdentifier] = useState(str(item.public_identifier)); const [webhook, setWebhook] = useState(Boolean(item.webhook_configured)); const provider = str(item.provider); return <div className="rounded-2xl border bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div><h2 className="font-black text-slate-950">{providerNames[provider] ?? provider}</h2><p className="mt-1 text-sm text-slate-500">{item.secret_configured ? "Clé serveur configurée" : "Clé serveur à ajouter dans Vercel"}</p></div><label className="flex items-center gap-2 text-sm font-bold"><input type="checkbox" checked={enabled} onChange={(e) => setEnabled(e.target.checked)} />Actif</label></div><label className="mt-4 block text-sm font-bold">Identifiant public<input value={identifier} onChange={(e) => setIdentifier(e.target.value)} placeholder={provider === "whatsapp" ? "+52…" : "ID public, compte ou modèle"} className="mt-2 w-full rounded-xl border px-4 py-3 font-normal" /></label><div className="mt-4 grid grid-cols-2 gap-3"><select value={environment} onChange={(e) => setEnvironment(e.target.value)} className="rounded-xl border bg-white px-3 py-2"><option value="sandbox">Test / Sandbox</option><option value="production">Production</option></select><label className="flex items-center justify-center gap-2 rounded-xl border px-3 py-2 text-sm font-bold"><input type="checkbox" checked={webhook} onChange={(e) => setWebhook(e.target.checked)} />Webhook prêt</label></div><button disabled={saving} onClick={() => onSave({ provider, enabled, environment, publicIdentifier: identifier, webhookConfigured: webhook })} className="mt-4 w-full rounded-xl bg-[#123f91] px-4 py-3 text-sm font-bold text-white">Enregistrer</button></div>; }
